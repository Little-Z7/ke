...
| 2026-09-20 | src/client/shell/ke_redact.rs（新增）、ke_model.rs、settings.rs、settings_overlay.rs、overlays.rs、state.rs、mouse.rs、shell.rs、src/ke/slash.rs | 配置改到 TUI 里完成：model 页加 provider 切换（openai/cli，切到 cli 后换成 command 一行输入，按 shell 规则解析 argv）与 panel_model；新增 redact 分页（[ke.redact] 六项开关，此前完全没有界面）；`settings_choice_count` 由写死的 8 改为按 `visible_fields()` 动态计算；修 apply 把手写的 `provider = "cli"` 悄悄写回 openai 的 bug |
| 2026-09-21 | src/update.rs、src/ke/slash.rs、src/client/shell/{ke_redact,ke_cmd}.rs | `ke update` 成功后改印真正的重启指引（旧 server 不停就一直渲染旧版本，而上游那份指引写死 `herdr`，用户若同时装了上游会停错 server，故按 ke 自己的命令名生成并加测试守住）；`/ke help` 文案跟上实际命令；新增 `/ke redact` 直接打开脱敏页 |
