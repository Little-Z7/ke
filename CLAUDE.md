...
- Flow: composer submit → local socket → resident processor (reversible mask / `done` when `@ke` is handed to the model) → `agent.prompt` or `pane.send_input`. The composer path and the three model-context paths in `answer.rs` all mask through the one session-level `Mapping` on `Shared.redaction`, so a placeholder means the same thing to the agent and to the model; `redact.rs` explains why that must stay true. The resident polls `agent.list` and writes `panel.json`/`chat.jsonl`; the client polls those files by mtime. **This adds no wire-protocol changes** — keep it that way.
